...
feeds.refreshUI = function() {
  ...
  if (options.get(options.Options.BADGE_TEXT_SHOWN)) {
    ...
    var bgcolor = nextEvent.feed.backgroundColor;
    var isInFive = moment(nextEvent.start).isBefore(moment().add(5, 'minutes'));

    background.updateBadge({
      'color': isInFive ? utils.darken('#d60501',10)  : utils.darken(bgcolor,40),
      'text': badgeText,
